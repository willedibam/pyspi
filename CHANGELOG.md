...
- **KSG dither was shared between identical columns.** The dither was keyed on a column's contents alone, so two identical columns drew the *same* noise and it never broke the degeneracy between them. With `x == y` exactly, every joint L∞ distance equals the marginal one, so the kth joint radius is the kth marginal radius and strict counting gives `n_x = n_y = k − 1`; the estimate collapses to **`ψ(N) − ψ(k)`**, with no dependence on the data. Measured **4.0397, 4.7341, 5.4279, 6.1213** nats at N = 200, 400, 800, 1600 with k = 4 — `ψ(N) − ψ(4)` to four decimals at every N — and identical for binary, four-level and rounded-Gaussian duplicates, against a true value of `H(X) = ln 2 = 0.693`. The key now includes a replica index, so each coordinate *occurrence* draws its own noise while distinct columns keep the content-determined draw that makes `bivariate` and `multivariate` agree bit-for-bit. Duplicate binary now returns 0.70–0.76 and duplicate four-level 1.40–1.43, converging with N. **Scope:** this changes only paths containing byte-identical normalised coordinate occurrences — duplicate or exactly collinear processes, and a source equal to its target. Ordinary `kraskov` results over distinct coordinates are unchanged, which is why no bundled baseline moved even though the dither key changed for every column: only the integer neighbour counts enter the estimate, and a 1e-8 perturbation does not move which points fall inside the radius (verified — the same pair returns the identical 15 significant figures under four different dither seeds).

- **KSG rescaling invariance broke on tied data.** The dither key was the normalised column's exact bytes, and `x` and `1000·x` normalise to columns differing by one ulp (1.1e-16). That flipped the key, re-drew the whole dither, and on *tied* data a different dither separates the ties differently: MI moved by **0.0375 nats** on a binary pair under a rescaling the estimator is supposed to be invariant to. The key is now taken from the column rounded to 12 decimals — far below anything distinguishing two series, far above float noise — so the invariance is exact on quantised data too. `kraskov` values on tied or quantised input can change; tie-free values do not, since only the integer neighbour counts enter the estimate.
...
- **Structural traits now win over config-declared directedness in both directions.** The rule was one-sided, so a family-level `directed` could sit on a statistic that declares itself undirected: `gd_*_rvalue` (symmetric by construction) came back both `undirected` and `directed` through YAML, and `filter_spis` answered either way for the same SPI.
