...
- **KSG dither was shared between identical columns.** The dither was keyed on a column's contents alone, so two identical columns drew the *same* noise and it never broke the degeneracy between them. On identical binary series the joint cloud collapsed onto the diagonal and the estimate reduced to `psi(k) − 2·psi(k+1) + psi(N)` — a function of N and k with no dependence on the data: **4.04, 4.73, 5.43, 6.12 nats at N = 200, 400, 800, 1600**, growing by ln 2 per doubling, and identical for binary, four-level and rounded-Gaussian duplicates, against a true value of `H(X) = ln 2 = 0.693`. The key now includes a replica index, so each coordinate *occurrence* draws its own noise while distinct columns keep the content-determined draw that makes `bivariate` and `multivariate` agree bit-for-bit. Duplicate binary now returns 0.70–0.76 and duplicate four-level 1.40–1.43, converging with N. **Every `kraskov` SPI's values change again.**
