...
- **KSG dither was shared between identical columns.** The dither was keyed on a column's contents alone, so two identical columns drew the *same* noise and it never broke the degeneracy between them. With `x == y` exactly, every joint L∞ distance equals the marginal one, so the kth joint radius is the kth marginal radius and strict counting gives `n_x = n_y = k − 1`; the estimate collapses to **`ψ(N) − ψ(k)`**, with no dependence on the data. Measured **4.0397, 4.7341, 5.4279, 6.1213** nats at N = 200, 400, 800, 1600 with k = 4 — `ψ(N) − ψ(4)` to four decimals at every N — and identical for binary, four-level and rounded-Gaussian duplicates, against a true value of `H(X) = ln 2 = 0.693`. Each coordinate occurrence now receives a separate deterministic replica dither. Duplicate binary returns 0.70–0.76 and duplicate four-level 1.40–1.43, converging with N.

- **KSG conditioning was not symmetric or reflection-covariant at its numerical tolerance.** Standardised coordinates were rounded to 12 decimals only for the dither key, then replicas were assigned in input order. Distinct near-identical coordinates could share a key while retaining different values: at N=100/seed=53, reversing a four-level `x` and `1000*x + 1e-11*noise` moved MI by **5.263e-4**. Reflection redrew the dither, moving tied binary/four-level MI by up to **0.07495** nats. KSG coordinates are now standardised after a cancellation-resistant origin shift, rounded to 12 decimals for both the estimator and key (maximum quantisation error `5e-13`, four orders below the `1e-8` dither), and keyed canonically with their reflection. Dither reflects with the coordinate; distinct key collisions are assigned by content order; exact replicas remain independently dithered. This makes the conditioned L∞ geometry deterministic, symmetric, process-permutation covariant, and invariant within the stated tolerance under nonzero affine marginal transformations. It does **not** claim finite-sample invariance under arbitrary nonlinear transformations. Tied/quantised and tolerance-boundary `kraskov` values can change; ordinary tie-free neighbour counts remain unchanged.
